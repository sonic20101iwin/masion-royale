...
async function pexelsSearch(termKey, query) {
  ...
  if (!res.ok) { console.log('PEXELS FAIL', termKey, res.status); return []; }
  ...
  return ids.slice(0, 14).map((id) => ({ src: pexelsUrl(id), srcType: PE, id }));
}
...
async function commonsSearch(termKey, q) {
  ...
  if (!res.ok) { console.log('COMMONS FAIL', termKey, res.status); return []; }
  ...
  for (const p of pages) {
    ...
    const isSvg = p.title.toLowerCase().endsWith('.svg') || p.title.toLowerCase().includes('.tif');
    if (isSvg) continue;
