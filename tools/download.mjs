...
const plan = {
  fashion: 6, models: 4, runway: 4, shoe: 6, heels: 6, sneaker: 6, watch: 4,
  bag: 6, jewelry: 6, diamond: 2, earrings: 3, dress: 6, gowns: 3, suit: 4,
  jacket: 3, stylishwoman: 3, sunglasses: 3, perfume: 4, makeup: 4,
  dining: 4, food: 3, dessert: 4, pastry: 2, champagne: 3, lounge: 5, barista: 4,
  interior: 6, escalator: 5, building: 5, hotellobby: 3, nightcity: 4,
  spa: 5, swimmingpool: 3, fitness: 4, cinema: 6, event: 4, car: 4,
  gift: 5, valet: 4, tie: 2, manwatch: 1, italian: 3, sushibar: 4, coffeeshop: 3,
};
...
const workers = 12;
const batches = [];
for (let i = 0; i < tasks.length; i += workers) batches.push(tasks.slice(i, i + workers));
...
for (const batch of batches) {
  await Promise.all(batch.map(async (t) => {
    try {
      ...
      if (!res.ok) { failures.push([t.key, t.name, res.status]); return; }
      const buf = Buffer.from(await res.arrayBuffer());
      if (buf.length < 2000 || (buf[0] !== 0xff || buf[1] !== 0xd8)) { failures.push([t.key, t.name, 'BAD:' + buf.length]); return; }
      fs.writeFileSync(t.dest, buf);
      done++;
    } catch (e) { failures.push([t.key, t.name, 'ERR']); }
  }));
...
fs.writeFileSync('tools/preview-manifest.json', JSON.stringify(manifest, null, 1));
